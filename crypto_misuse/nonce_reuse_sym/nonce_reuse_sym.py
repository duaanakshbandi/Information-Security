...
# enter your code here
    cipher1 = bytesToBits(cts[0])
    cipher2 = bytesToBits(cts[1])
    p1XORp2 = []
    for bit1, bit2 in zip(cipher1, cipher2):
        p1XORp2.append(bit1 ^ bit2)
    l = lfsr.LFSR(LFSR_LEN, p1XORp2, LFSR_TAPS)
    p2XORp3 = l.output(LFSR_LEN)
    p2XORp3 = l.output(LFSR_LEN)
    cipher3 = []
    for bit1, bit2 in zip(p2XORp3, cipher2):
        cipher3.append(bit1 ^ bit2)

    next_nonce = nonces[0]
    next_ct = bitsToBytes(cipher3)
########################################################################
