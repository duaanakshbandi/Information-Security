...
def show_logged_in():
    if ('session_id' in request.cookies):
        ...
        code = ""
        if (user["role"] == "admin"):
            code = "<hr />\n<h2>Promote:</h2>\n<form method=\"post\" action=\"/promote\">\n<input type=\"hidden\" value=\"" + request.cookies['csrf_token'] + "\" name=\"csrf_token\">\n<input name=\"username\" placeholder=\"username\" />\n<input type=\"submit\" name=\"Promote\" value=\"Promote\"/>\n</form>\n"
            with open("flag.txt", 'r') as f:
                flag = f.readline().rstrip("\n")
        with open('./templates/logged_in.html', 'r') as f:
            HTML = f.read()
        template = Template(HTML)
        return template.substitute(user=user["username"], flag=flag, code=code, csrf_token=request.cookies['csrf_token'])
    else:
        redirect_to_board = redirect('/')
        response = INFOSEC_web.make_response(redirect_to_board)
        return response
