...
class Utils:
    ...
    #---------------------------------------------------------------------------
    def store_user(self, user_name, password, creation_time=int(time.time())):
        ...
        cur.execute(
            ...
            "  VALUES (%(username)s,%(password)s,FROM_UNIXTIME(%(creation_time)s));",
            params)
    ...
    def create_and_store_session_id(self, username):
        ...
        cur.execute("SELECT id, UNIX_TIMESTAMP(creation_time) FROM user WHERE username = %(username)s;", params)
        rv = cur.fetchall()
        if (len(rv) == 0): return
        user_id = rv[0][0]
        creation_time = rv[0][1]
        timestamp = int(time.time())
        hasher = SHA256.new()
        s = struct.pack('<Qi', creation_time, timestamp)
        hasher.update(s)
        ...
        cur.execute(
            "INSERT INTO session(id,user_id,csrf_token,timestamp) VALUES "
            "  (%(session_id)s,"
            "  (SELECT id FROM user WHERE username = %(username)s),%(csrf_token)s,FROM_UNIXTIME(%(timestamp)s));",
            params)
        self.mysql.connection.commit()
    ...
    def get_account(self, user_name):
        cur = self.mysql.connection.cursor()
        params = {'username': user_name}
        cur.execute("SELECT id, role FROM user WHERE username = %(username)s;", params)
        rv = cur.fetchall()
        if (len(rv) == 0):
            return {'user_id': "", 'role': "", 'user_name': "", 'last_login': ""}
        params = {'user_id': rv[0][0]}
        cur.execute("SELECT timestamp FROM session WHERE user_id = %(user_id)s", params)
        rv_session = cur.fetchall()
        if (len(rv_session) == 0):
            last_login = ""
        else:
            last_login = rv_session[0][0].strftime("%Y-%m-%d %H:%M:%S")
        params = {'user_id': rv[0][0],
                  'role': rv[0][1],
                  'user_name': user_name,
    ...
    def get_posts_from_db(self, filter):
        cur = self.mysql.connection.cursor()
        query = "SELECT p.timestamp,u.username,UNIX_TIMESTAMP(u.creation_time),p.text FROM post p INNER JOIN user u ON p.user_id = u.id"
        if (filter):
    ...
    def print_posts(self, filter=None):
        ...
        for i in range(0, len(posts)):
            ...
            html = html + self.substitute_xss(posts[i][1]) + "</td><td>"
            if (time.time() - posts[i][2]) < (24 * 60 * 60):
                html = html + "Yes"
            else:
                html = html + "No"
            html = html + "</td><td>" + self.substitute_xss(posts[i][3]) + "</td></tr>\n"
        return html

    #---------------------------------------------------------------------------
    def insert_post_to_db(self, user_id, message, timestamp=int(time.time())):
        ...
        cur.execute("INSERT INTO post(user_id,text,timestamp) "
                    "  VALUES (%(user_id)s,%(text)s,FROM_UNIXTIME(%(timestamp)s));", params)
        self.mysql.connection.commit()
