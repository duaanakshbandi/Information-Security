...
-- -----------------------------------------------------
-- Table `INFOSEC_DB`.`image`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `INFOSEC_DB`.`image` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `INFOSEC_DB`.`image` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(64) NOT NULL,
  `tag` VARCHAR(64) NOT NULL,
  `content` VARCHAR(2000) NOT NULL,
  `security_feature` BOOLEAN NOT NULL,
  `creation_time` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`))
ENCRYPTION = 'Y',
ENGINE = InnoDB;
...
GRANT SELECT,INSERT,DELETE,UPDATE ON `INFOSEC_DB`.`image` TO 'db_user'@'%';
