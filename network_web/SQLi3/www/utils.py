...
class Utils:
    ...
    def get_posts_from_db(self):
        cur = self.mysql.connection.cursor()
        cur.execute("SELECT p.timestamp,u.username,p.text,p.id FROM "
                    "  post p INNER JOIN user u ON p.user_id = u.id ")
        return cur.fetchall()

    #---------------------------------------------------------------------------
    def get_likes_for_post(self, id):
        cur = self.mysql.connection.cursor()
        params = {'id': id}
        cur.execute("SELECT l.id FROM "
                    "  likes l INNER JOIN post p ON l.post_id = p.id "
                    "  where p.id = %(id)s", params)
    ...
    def print_posts(self):
        ...
        for i in range(0, len(posts)):
            ...
            html = html + "</td><td>" + self.substitute_xss(str(posts[i][2])) + "</td><td>"
            html = html + str(self.get_likes_for_post(posts[i][3])) + "</td><td>"
            html = html + "<input type=\"checkbox\" name=" +  self.substitute_xss(str(posts[i][3])) + " value=\"true\" /></td></tr>\n"
        return html
    ...
    def like(self, user_id, post_ids):
        ...
        for id in post_ids:
            ...
            cur.execute("SELECT * from likes WHERE user_id = %(user_id)s and post_id = %(post_id)s", params)
            if (len(cur.fetchall()) == 0):
                cur.execute("INSERT INTO likes(user_id, post_id) "
                            " VALUES (%(user_id)s, %(post_id)s);", params)
        self.mysql.connection.commit()


    #---------------------------------------------------------------------------
    def unlike(self, user_id, post_ids):
        queries = ""
        for id in post_ids:
            queries = queries + "DELETE FROM likes WHERE user_id = \'%s\' and post_id = \'%s\';"%(str(user_id), str(id))
        if (queries != ""):
            cur = self.mysql.connection.cursor()
            cur.execute(queries)
            cur.close()
