...
#-------------------------------------------------------------------------------
@INFOSEC_web.route('/promote', methods=['POST'])
def promote():
    if ('session_id' in request.cookies):
        ...
        utils.update_user(request.form["username"])
...
def show_logged_in():
    if ('session_id' in request.cookies):
        ...
        if (user["role"] == "admin"):
            ...
            messages = "<table class=\"text-white\">\n<thead>\n<tr align=\"center\">\n  <th>Date</th>\n  <th>User</th>\n  <th>Message</th>\n</tr>\n</thead>\n<tbody>\n"
        ...
        return template.substitute(user=utils.substitute_xss(user["username"]), code=code, flag=flag, messages=messages, csrf_token=request.cookies['csrf_token'])
    else:
        redirect_to_board = redirect('/')
        response = INFOSEC_web.make_response(redirect_to_board)
        return response
